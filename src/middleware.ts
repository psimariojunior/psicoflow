  ...
  },
  {
    callbacks: {
      authorized: ({ token, req }) => {
        const pathname = req.nextUrl.pathname
        if (pathname.startsWith("/api/auth") || pathname.startsWith("/api/webrtc") || pathname.startsWith("/api/livekit") || pathname.startsWith("/api/cron") || pathname.startsWith("/_next") || pathname.startsWith("/static")) {
          return true
        }
